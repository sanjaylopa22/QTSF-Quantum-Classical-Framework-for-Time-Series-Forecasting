"""
VQF-F on ETTm2  —  TSLib-compatible pipeline  (GPU edition, BATCHED v2)
========================================================================
...
Usage
─────
  python vqff_ettm2_fast.py --data_path /path/to/ETTm2.csv \\
...
class Config:
    ...
    num_workers   = 4
    pin_memory    = True
    checkpoints   = "./checkpoints_vqff_ettm2_fast/"
    use_lightning = False
...
def run_horizon(df, pred_len, cfg, device):
    print(f"\n{'='*72}")
    print(f"  VQF-F (broadcast) | ETTm2 | pred_len={pred_len} | "
    ...
    train_ds = ETTm2Dataset(df, "train", cfg)
    scaler   = train_ds.scaler
    val_ds   = ETTm2Dataset(df, "val",   cfg, scaler)
    test_ds  = ETTm2Dataset(df, "test",  cfg, scaler)
    ...
    ckpt = os.path.join(cfg.checkpoints, f"vqff_ettm2_fast_pl{pred_len}.pt")
...
def plot_all(results, cfg):
    ...
    for row, res in enumerate(results):
        ...
        ax.set_xlabel("Step (15-min)"); ax.set_ylabel("OT value")
        ax2=axes[row][1]
    ...
    plt.suptitle("VQF-F (broadcast) on ETTm2  —  GPU edition",
                 fontsize=13,fontweight="bold",y=1.01)
    plt.tight_layout()
    plt.savefig("vqff_ettm2_fast_results.png",dpi=150,bbox_inches="tight")
    print("\n  Plot saved → vqff_ettm2_fast_results.png")
    plt.close()

def print_summary(results):
    print("\n"+"="*72)
    print(f"{'VQF-F (broadcast) on ETTm2 — Summary (original scale)':^72}")
    print("="*72)
...
def main():
    pa = argparse.ArgumentParser(
        description="VQF-F on ETTm2 — broadcast batched GPU edition",
    ...
    print("="*72)
    print("  VQF-F (broadcast): Variational Quantum Forecaster on ETTm2")
    ...
    print(f"\n  ETTm2 shape: {df.shape}  columns: {list(df.columns)}")
