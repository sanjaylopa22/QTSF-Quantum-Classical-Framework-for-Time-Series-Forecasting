"""
VQF-F on ETTh1  —  TSLib-compatible pipeline  (GPU edition, BATCHED)
======================================================================
...
vectorised in ONE circuit call per channel instead of B sequential calls.
  ...
  ETTh1 dataset  (TSLib Dataset_ETT_hour — fixed monthly borders)
  ────────────────────────────────────────────────────────────────
  • ETTh1.csv columns : date, HUFL, HULL, MUFL, MULL, LUFL, LULL, OT
...
─────
  ...
  python vqff_etth1_fast.py --data_path /path/to/ETTh1.csv \\
...
class Config:
    ...
    num_workers   = 4
    pin_memory    = True
    checkpoints   = "./checkpoints_vqff_etth1_fast/"
    use_lightning = False
...
def run_horizon(df, pred_len, cfg, device):
    print(f"\n{'='*72}")
    print(f"  VQF-F (broadcast) | ETTh1 | pred_len={pred_len} | "
    ...
    train_ds = ETTh1Dataset(df, "train", cfg)
    scaler   = train_ds.scaler
    val_ds   = ETTh1Dataset(df, "val",   cfg, scaler)
    test_ds  = ETTh1Dataset(df, "test",  cfg, scaler)
    ...
    ckpt = os.path.join(cfg.checkpoints, f"vqff_etth1_fast_pl{pred_len}.pt")
...
def plot_all(results, cfg):
    ...
    plt.suptitle("VQF-F (broadcast) on ETTh1  —  GPU edition",
                 fontsize=13,fontweight="bold",y=1.01)
    plt.tight_layout()
    plt.savefig("vqff_etth1_fast_results.png",dpi=150,bbox_inches="tight")
    print("\n  Plot saved → vqff_etth1_fast_results.png")
    plt.close()

def print_summary(results):
    print("\n"+"="*72)
    print(f"{'VQF-F (broadcast) on ETTh1 — Summary (original scale)':^72}")
    print("="*72)
...
def main():
    pa = argparse.ArgumentParser(
        description="VQF-F on ETTh1 — broadcast batched GPU edition",
    ...
    print("="*72)
    print("  VQF-F (broadcast): Variational Quantum Forecaster on ETTh1")
    ...
    print(f"\n  ETTh1 shape: {df.shape}  columns: {list(df.columns)}")
