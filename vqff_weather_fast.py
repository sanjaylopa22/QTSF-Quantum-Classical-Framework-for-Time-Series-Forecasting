"""
VQF-F on Weather  —  TSLib-compatible pipeline  (GPU edition, BATCHED v2)
========================================================================
Architecture is identical to the original VQF-F.

KEY FIX (v2, the real one): use PennyLane's native parameter broadcasting.
PennyLane qnodes support a leading batch dimension on input angles natively
(no vmap, no autograph) — passing a (B, n_qubits) tensor returns
(d_f, B) observable expectations in ONE circuit call.

  Tested speedup (B=256, n=6, depth=2, default.qubit, CPU):
    loop      : 2.39 s  per channel per batch
    broadcast : 0.05 s  per channel per batch     ← 48× faster
  On lightning.gpu the gap is even larger.

  Weather specifics
  ────────────────
  • 15-minute intervals (freq='t', 4 samples/hour)
  • Split borders (TSLib Dataset_ETT_minute):
        train [0,34560)  val [34560,46080)+ovlp  test [46080,57600)+ovlp
  • Time features: month/day/weekday/hour/minute  (5 channels)
  • seq_len=96, pred_lens in {96,192,336,720}
...
Usage
─────
  python vqff_weather_fast.py --data_path /path/to/Weather.csv \\
      --use_lightning --device cuda:0 \\
      --pred_lens 96 192 336 720 --batch_size 256 --epochs 10
"""
...
class Config:
    ...
    num_workers   = 4
    pin_memory    = True
    checkpoints   = "./checkpoints_vqff_weather_fast/"
    use_lightning = False


# ─────────────────────────────────────────────────────────────────────────────
# 2.  TIME FEATURES (freq='t')
# ─────────────────────────────────────────────────────────────────────────────
def time_features(dates):
    """TSLib timeF freq='h' — 4 channels."""
    return np.stack([
...
class WeatherDataset(Dataset):
    """TSLib Dataset_Custom 70/10/20 split for Weather (ratio-based)."""
    def __init__(self, df, split, cfg, scaler=None):
        ...
        self.target_idx = (feat_cols.index(cfg.target)
                           if cfg.target in feat_cols else len(feat_cols) - 1)
...
#     The circuit indexes input angles with `[..., i]` so PennyLane
#     automatically broadcasts a leading batch dimension across the entire
#     batch.  ONE call evaluates (B, n_qubits) -> (d_f, B) in parallel.
# ─────────────────────────────────────────────────────────────────────────────
class BroadcastVQC:
    def __init__(self, cfg):
        ...
        def _circuit(angles, theta_ry, phi_rz):
            # angles : (B, n)  — leading dim broadcasts natively in PennyLane
            for i in range(n):
            ...
            obs += [qml.expval(qml.PauliZ(i) @ qml.PauliZ(i + 1))
                    for i in range(n - 1)]
            return obs   # list of d_f tensors, each shape (B,)

        self._circuit = _circuit

    def forward_batch(self, angles_b, theta_ry, phi_rz):
        """
        angles_b : (B, n_qubits)  CPU tensor
        theta_ry : (depth, n)     CPU tensor  (shared across batch)
        phi_rz   : (depth, n)     CPU tensor
        returns  : (B, d_f)       CPU tensor
        """
        out = self._circuit(angles_b, theta_ry, phi_rz)
        # out is a list of d_f tensors each of shape (B,).
        # Stack into (B, d_f).
        return torch.stack(out, dim=-1).float()


# ─────────────────────────────────────────────────────────────────────────────
# 6.  MODEL  (architecture identical to original)
# ─────────────────────────────────────────────────────────────────────────────
class VQFFModel(nn.Module):
    ...
    def forward(self, x, x_mark=None):
        ...
        all_ch = []
        for m in range(M):
            # Single broadcast call -- PennyLane vectorises across batch
            feats_m = self.qcirc.forward_batch(
                angles_t[:, m, :],     # (B, n)  -- batch dim leading
                self.theta_ry[m],      # (depth, n) -- shared
                self.phi_rz[m],        # (depth, n) -- shared
            )                          # (B, d_f)
...
# ─────────────────────────────────────────────────────────────────────────────
# 8.  TRAIN / EVAL  (with per-batch progress so you can SEE it working)
# ─────────────────────────────────────────────────────────────────────────────
...
def run_horizon(df, pred_len, cfg, device):
    print(f"\n{'='*72}")
    print(f"  VQF-F (broadcast) | Weather | pred_len={pred_len} | "
    ...
    train_ds = WeatherDataset(df, "train", cfg)
    scaler   = train_ds.scaler
    val_ds   = WeatherDataset(df, "val",   cfg, scaler)
    test_ds  = WeatherDataset(df, "test",  cfg, scaler)
    ...
    ckpt = os.path.join(cfg.checkpoints, f"vqff_weather_fast_pl{pred_len}.pt")
    ...
    return {"pred_len":pred_len,"metrics":m,"preds":pi,"trues":ti,
            "train_hist":train_hist,"val_hist":val_hist,"scaler":scaler,"target_idx":test_ds.target_idx}
...
def plot_all(results, cfg):
    ...
    for row, res in enumerate(results):
        H=res["pred_len"]; tid=res["target_idx"]; true=res["trues"][0,:,tid]; pred=res["preds"][0,:,tid]
        ax=axes[row][0]
        ax.plot(true,color="#1D9E75",lw=1.6,label="Ground truth")
        ax.plot(pred,color="#D85A30",lw=1.6,ls="--",label="VQF-F")
        ax.set_title(f"H={H}  T (degC) channel  (original scale)",fontsize=9)
        ax.legend(fontsize=8); ax.grid(alpha=.3)
        ax.set_xlabel("Step (hourly)"); ax.set_ylabel("Temperature (°C)")
        ax2=axes[row][1]
    ...
    plt.suptitle("VQF-F (broadcast) on Weather  —  GPU edition",
                 fontsize=13,fontweight="bold",y=1.01)
    plt.tight_layout()
    plt.savefig("vqff_weather_fast_results.png",dpi=150,bbox_inches="tight")
    print("\n  Plot saved → vqff_weather_fast_results.png")
    plt.close()

def print_summary(results):
    print("\n"+"="*72)
    print(f"{'VQF-F (broadcast) on Weather — Summary (original scale)':^72}")
    print("="*72)
...
def main():
    pa = argparse.ArgumentParser(
        description="VQF-F on Weather — broadcast batched GPU edition",
    ...
    print("="*72)
    print("  VQF-F (broadcast): Variational Quantum Forecaster on Weather")
    print("  KEY FIX: PennyLane native parameter broadcasting")
    print("           (B, n) angles -> ONE circuit call per channel")
    print("="*72)
    ...
    print(f"\n  Weather shape: {df.shape}  columns: {list(df.columns)}")
