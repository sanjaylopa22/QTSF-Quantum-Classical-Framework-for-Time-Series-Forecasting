"""
VQF-F on Exchange Rate  —  TSLib-compatible pipeline  (GPU edition, BATCHED)
=============================================================================
...
vectorised in ONE circuit call per channel instead of B sequential calls.
  ...
  Dataset specifics  (preserved from original vqff_exchange.py)
  ──────────────────────────────────────────────────────────────
  • exchange_rate.csv  (TSLib Exchange Rate — daily, 8 countries vs USD)
  • 7588 daily rows  (1990-01-01 to 2010-12-31)
  • 8 channels  (enc_in = c_out = 8)
  • Daily frequency → 3 time features only: month / day / weekday
    (no hour, since data is daily)
  • Date column auto-detected: 'date', 'unnamed: 0', or synthetic index
  • TSLib Dataset_Custom ratio split:
        num_train = int(N * 0.7)
        num_test  = int(N * 0.2)
...
─────
  ...
  python vqff_exchange_fast.py \\
      --data_path ./dataset/exchange_rate/exchange_rate.csv \\
      --device cuda:0 --batch_size 256 --epochs 10
"""
...
class Config:
    ...
    features      = "M"
    target        = "OT"            # last channel by convention
    freq          = "d"             # daily
    enc_in        = 8               # 8 currencies
    c_out         = 8
    train_ratio   = 0.7
    val_ratio     = 0.1
    seq_len       = 96
    ...
    num_workers   = 4
    pin_memory    = True
    checkpoints   = "./checkpoints_vqff_exchange_fast/"
    use_lightning = False           # default.qubit is faster for n=6


# ─────────────────────────────────────────────────────────────────────────────
# 2.  TIME FEATURES — daily, 3 channels (month / day / weekday)
# ─────────────────────────────────────────────────────────────────────────────
def time_features_daily(dates):
    """Returns (N, 3) float32: month, day, weekday — all in [-0.5, 0.5]."""
    return np.stack([
...
# ─────────────────────────────────────────────────────────────────────────────
# 3.  DATASET  —  TSLib Dataset_Custom ratio split (preserved from original)
# ─────────────────────────────────────────────────────────────────────────────
class ExchangeRateDataset(Dataset):
    def __init__(self, df, split, cfg, scaler=None):
        ...
        # detect date column — handles 'date', 'unnamed: 0', or no header
        date_col = None
        for c in df.columns:
            if str(c).lower() in ("date", "unnamed: 0", ""):
                date_col = c
                break
        ...
        self.time_mark_full = time_features_daily(dates)
        data_raw = df[feat_cols].values.astype(np.float32)
        N        = len(data_raw)

        # ratio split (TSLib Dataset_Custom)
        num_train = int(N * cfg.train_ratio)
        num_test  = int(N * (1.0 - cfg.train_ratio - cfg.val_ratio))
        num_val   = N - num_train - num_test
        borders = {
            "train": (0,                          num_train),
            "val":   (num_train - cfg.seq_len,    num_train + num_val),
            "test":  (N - num_test - cfg.seq_len, N),
        }
        s, e = borders[split]
        s = max(s, 0); e = min(e, N)
        ...
        if scaler is None:
            ...
            self.scaler.fit(data_raw[ts:te])
        else:
            self.scaler = scaler

        scaled         = self.scaler.transform(data_raw)
        self.data      = scaled[s:e]
        self.time_mark = self.time_mark_full[s:e]
        self.feat_cols = feat_cols

        # update enc_in / c_out from actual CSV
        cfg.enc_in = len(feat_cols)
        cfg.c_out  = len(feat_cols)

        print(f"    [{split}]  rows: {s}..{e}  ({e-s} total)  "
              f"channels: {len(feat_cols)}")
...
#     `qml.RY(angles[..., i])` — leading batch dim broadcasts natively.
...
# ─────────────────────────────────────────────────────────────────────────────
# 6.  MODEL  (architecture UNCHANGED)
# ─────────────────────────────────────────────────────────────────────────────
class VQFFModel(nn.Module):
    def __init__(self, cfg, pred_len):
        ...
        self.cfg = cfg
        self.n   = cfg.N_QUBITS
        self.M   = cfg.enc_in            # 8 for Exchange
...
def run_horizon(df, pred_len, cfg, device):
    print(f"\n{'='*72}")
    print(f"  VQF-F (broadcast) | Exchange Rate | pred_len={pred_len} | "
          f"enc_in={cfg.enc_in}")
    print(f"  seq_len={cfg.seq_len} | batch={cfg.batch_size} | device={device}")
    print(f"  n_qubits={cfg.N_QUBITS} | depth={cfg.CIRCUIT_DEPTH} | "
          f"AMP={cfg.use_amp and device.startswith('cuda')}")
    print(f"{'='*72}", flush=True)

    train_ds = ExchangeRateDataset(df, "train", cfg)
    scaler   = train_ds.scaler
    val_ds   = ExchangeRateDataset(df, "val",   cfg, scaler)
    test_ds  = ExchangeRateDataset(df, "test",  cfg, scaler)
    ...
    if min(len(train_ds), len(val_ds), len(test_ds)) <= 0:
        raise ValueError(
            f"Not enough rows for pred_len={pred_len} after seq_len overlap. "
            f"Try a smaller --pred_lens or --seq_len.")
    ...
    ckpt = os.path.join(cfg.checkpoints, f"vqff_exchange_fast_pl{pred_len}.pt")
...
def plot_all(results, cfg):
    ...
    for row, res in enumerate(results):
        ...
        ax.set_title(f"H={H}  Last channel, sample 0  (original scale)",
                     fontsize=9)
        ax.legend(fontsize=8); ax.grid(alpha=.3)
        ax.set_xlabel("Step (daily)"); ax.set_ylabel("Exchange rate")
        ...
        ax2.set_title(f"H={H}  Scatter (all 8 channels)", fontsize=9)
    ...
    plt.suptitle("VQF-F (broadcast) on Exchange Rate  —  GPU edition",
                 fontsize=13, fontweight="bold", y=1.01)
    plt.tight_layout()
    plt.savefig("vqff_exchange_fast_results.png", dpi=150, bbox_inches="tight")
    print("\n  Plot saved → vqff_exchange_fast_results.png")
    plt.close()


def print_summary(results):
    print("\n" + "=" * 72)
    print(f"{'VQF-F (broadcast) on Exchange Rate — Summary (original scale)':^72}")
    print("=" * 72)
    ...
    if len(results) > 1:
        print("-" * 72)
        print(f"{'Avg':>6} | "
              f"{np.mean([r['metrics']['MSE']  for r in results]):>10.4f} | "
              f"{np.mean([r['metrics']['MAE']  for r in results]):>10.4f} | "
              f"{np.mean([r['metrics']['RMSE'] for r in results]):>10.4f} |")
    print("=" * 72)
...
def main():
    pa = argparse.ArgumentParser(
        description="VQF-F on Exchange Rate — broadcast batched GPU edition",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter)
    pa.add_argument("--data_path",     type=str,   required=True,
                    help="Path to exchange_rate.csv  (TSLib: "
    ...
    if not os.path.isfile(cfg.data_path):
        raise FileNotFoundError(
            f"exchange_rate.csv not found at '{cfg.data_path}'.\n"
            f"Expected: dataset/exchange_rate/exchange_rate.csv in TSLib.")
    print(f"\n  Loading: {os.path.abspath(cfg.data_path)}")
    df = pd.read_csv(cfg.data_path)
    print(f"  Shape: {df.shape}  columns: {list(df.columns)}")

    print("=" * 72)
    print("  VQF-F (broadcast): Variational Quantum Forecaster on Exchange Rate")
    ...
    print(f"  freq       : daily  (3 time features: month/day/weekday)")
    print(f"  Split      : 70/10/20 %  (TSLib Dataset_Custom ratio)")
